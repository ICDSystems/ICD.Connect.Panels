...
namespace ICD.Connect.Panels.Server
{
		...
		where TSettings : IPanelServerDeviceSettings, new()
	{
		...
		private void InitializeClient(uint clientId)
		{
			...
			string localTimeMessage = JsonUtils.SerializeMessage(w => w.WriteValue(IcdEnvironment.GetLocalTime().ToString("O")), TIME_MESSAGE);
			...
			try
			{
				...
				{
					string sigMessage = JsonUtils.SerializeMessage(sig.Serialize, SIG_MESSAGE);
